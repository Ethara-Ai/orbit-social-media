...
const ConnectionsTab = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full px-4 pb-6 space-y-6">
      ...
      <div
        className={`bg-white dark:bg-neutral-800 border border-slate-200 dark:border-neutral-700 ${BORDER_RADIUS.cardSmall} overflow-hidden`}
      >
        {/* Page Header */}
        <div className="px-4 pt-4 pb-3 border-b border-slate-100 dark:border-neutral-700">
          <h1 className="text-[20px] font-bold text-slate-900 dark:text-white mb-1">Connections</h1>
          <p className="text-[14px] text-slate-600 dark:text-neutral-400">
            Manage your network and discover new connections
          </p>
        </div>

        {/* My Connections Section Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-neutral-700 bg-slate-50/50 dark:bg-neutral-800/50">
          <div className="flex items-center gap-2">
            <Users className="w-5 h-5 text-slate-700 dark:text-neutral-300" />
            <h2 className="text-base font-semibold text-slate-900 dark:text-white">
              My Connections
            </h2>
          </div>
          <span
            className={`text-sm text-slate-500 dark:text-neutral-400 bg-slate-100 dark:bg-neutral-700 px-3 py-1 ${BORDER_RADIUS.badge}`}
          >
        ...
        {friends.length > 0 ? (
          ...
          <div className="text-center py-12 px-4">
            ...
            <p className="text-slate-600 dark:text-neutral-400">No connections yet</p>
          </div>
        )}
      </div>
      ...
      >
        {/* Section Header */}
        <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100 dark:border-neutral-700 bg-slate-50/50 dark:bg-neutral-800/50">
          <UserPlus className="w-5 h-5 text-slate-700 dark:text-neutral-300" />
          <h2 className="text-base font-semibold text-slate-900 dark:text-white">
            People You May Know
          </h2>
        </div>
        ...
        {suggestedUsers.length > 0 ? (
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {suggestedUsers.map((user, index) => (
              <motion.div
                key={user.id}
                className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-neutral-700/50 transition-colors"
                initial={{ opacity: 0, x: -10 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: index * 0.03 }}
              >
          ...
          <div className="text-center py-12 px-4">
            <UserPlus className="w-12 h-12 text-slate-300 dark:text-neutral-600 mx-auto mb-3" />
            <p className="text-slate-600 dark:text-neutral-400">No suggestions at the moment</p>
          </div>
        )}
      </div>
    </div>
