...
const ConversationsList = ({
  ...
  return (
    <div
      className={`w-full sm:w-80 border-r border-slate-200 dark:border-neutral-700 flex flex-col max-w-[100vw] overflow-x-hidden transition-colors ${
...
const ConversationsHeader = ({ searchQuery, setSearchQuery }) => {
  return (
    <div className="p-3 sm:p-4 border-b border-slate-100 dark:border-neutral-700 transition-colors">
      ...
      <div className="relative">
        <input
          ...
          className={`w-full h-9 sm:h-10 pl-9 sm:pl-10 pr-3 sm:pr-4 bg-slate-100 dark:bg-neutral-800 border-0 ${BORDER_RADIUS.input} text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-hidden focus:ring-2 focus:ring-orange-500 focus:bg-white dark:focus:bg-slate-700 transition-all`}
        />
        <Search className="absolute left-3 sm:left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-neutral-500" />
      </div>
    </div>
  );
};
...
const ConversationItem = memo(function ConversationItem({ conversation, isActive, onClick }) {
  ...
  return (
    <motion.div
      ...
      className={`flex items-center gap-2 sm:gap-3 p-3 sm:p-4 cursor-pointer transition-all border-b border-slate-50 dark:border-neutral-800 ${
        isActive
      ...
      <div className="flex-1 min-w-0">
        ...
        <p
          className={`text-xs truncate transition-colors ${
            hasUnread
              ? 'text-slate-800 dark:text-neutral-200 font-semibold'
...
const EmptySearchState = () => {
  return (
    <div className="text-center py-8 sm:py-12 px-4">
      <Search className="w-8 h-8 sm:w-10 sm:h-10 text-slate-300 dark:text-neutral-600 mx-auto mb-2 sm:mb-3" />
