    ...
    <div className="p-2 sm:p-4 border-t border-slate-100 dark:border-neutral-700 bg-white dark:bg-neutral-900 transition-colors">
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        <motion.button
          ...
          className={`p-2 sm:p-2.5 ${BORDER_RADIUS.button} hover:bg-slate-100 dark:hover:bg-neutral-800 transition-colors text-slate-500 dark:text-neutral-400 shrink-0 cursor-pointer`}
          whileTap={{ scale: 0.95 }}
        >
  ...
  return (
    <div className="mb-2 sm:mb-3 relative inline-block">
      <img
        src={attachment}
        alt="Attachment"
        className="w-16 h-16 sm:w-24 sm:h-24 object-cover rounded-lg sm:rounded-xl border border-slate-200 dark:border-neutral-600"
      />
      <button
        onClick={onRemove}
        className={`absolute -top-1.5 -right-1.5 sm:-top-2 sm:-right-2 bg-slate-900 dark:bg-neutral-600 text-white ${BORDER_RADIUS.button} p-0.5 sm:p-1 hover:bg-slate-700 dark:hover:bg-slate-500 transition-colors cursor-pointer`}
      >
        <X className="w-3 h-3" />
      </button>
    </div>
