...
vi.mock('../common/Badge', () => ({
  default: ({ count, variant, className }) => (
    <span data-testid="badge" data-count={count} data-variant={variant} className={className}>
      {count}
    </span>
  ),
}));
...
describe('NavigationItems', () => {
  ...
  describe('click handling', () => {
    ...
    it('should call markNotificationsAsRead when notifications is clicked', () => {
      ...
      render(
        <NavigationItems {...defaultProps} markNotificationsAsRead={markNotificationsAsRead} />
      );
  ...
  describe('notification badge', () => {
    it('should show notification badge when notificationCount > 0', () => {
      ...
      const notificationBadge = badges.find((badge) => badge.getAttribute('data-count') === '5');
    ...
    it('should not show notification badge when notificationCount is 0', () => {
      ...
      const badges = screen.getAllByTestId('badge');
      const notificationBadge = badges.find((badge) => badge.getAttribute('data-count') === '0');
      expect(notificationBadge).toBeUndefined();
    });

    it('should show danger variant for notification badge when inactive', () => {
      render(<NavigationItems {...defaultProps} activeTab="feed" notificationCount={5} />);
      const badges = screen.getAllByTestId('badge');
      const notificationBadge = badges.find((badge) => badge.getAttribute('data-count') === '5');
    ...
    it('should show light variant for notification badge when active', () => {
      render(<NavigationItems {...defaultProps} activeTab="notifications" notificationCount={5} />);
      const badges = screen.getAllByTestId('badge');
      const notificationBadge = badges.find((badge) => badge.getAttribute('data-count') === '5');
  ...
  describe('messages badge', () => {
    it('should show messages badge when totalUnreadMessages > 0', () => {
      ...
      const badges = screen.getAllByTestId('badge');
      const messagesBadge = badges.find((badge) => badge.getAttribute('data-count') === '3');
    ...
    it('should not show messages badge when totalUnreadMessages is 0', () => {
      ...
      const badges = screen.getAllByTestId('badge');
      const messagesBadge = badges.find((badge) => badge.getAttribute('data-count') === '0');
      expect(messagesBadge).toBeUndefined();
    });

    it('should show primary variant for messages badge when inactive', () => {
      render(<NavigationItems {...defaultProps} activeTab="feed" totalUnreadMessages={3} />);
      const badges = screen.getAllByTestId('badge');
      const messagesBadge = badges.find((badge) => badge.getAttribute('data-count') === '3');
    ...
    it('should show light variant for messages badge when active', () => {
      render(<NavigationItems {...defaultProps} activeTab="messages" totalUnreadMessages={3} />);
      const badges = screen.getAllByTestId('badge');
      const messagesBadge = badges.find((badge) => badge.getAttribute('data-count') === '3');
  ...
  describe('edge cases', () => {
    ...
    it('should handle large notification count', () => {
      ...
      const badges = screen.getAllByTestId('badge');
      const notificationBadge = badges.find((badge) => badge.getAttribute('data-count') === '999');
      expect(notificationBadge).toBeInTheDocument();
    });
    ...
    it('should handle large messages count', () => {
      ...
      const badges = screen.getAllByTestId('badge');
      const messagesBadge = badges.find((badge) => badge.getAttribute('data-count') === '500');
      expect(messagesBadge).toBeInTheDocument();
    });

    it('should handle special characters in label', () => {
      const specialItems = [{ id: 'test', label: 'Test & More <Items>', icon: MockIcon }];
  ...
  describe('both badges visible', () => {
    it('should show both notification and messages badges when both have counts', () => {
      render(<NavigationItems {...defaultProps} notificationCount={5} totalUnreadMessages={3} />);
      const badges = screen.getAllByTestId('badge');
      expect(badges).toHaveLength(2);
    });

    it('should show no badges when both counts are 0', () => {
      render(<NavigationItems {...defaultProps} notificationCount={0} totalUnreadMessages={0} />);
