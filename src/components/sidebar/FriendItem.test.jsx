...
describe('FriendItem', () => {
  ...
  describe('online status', () => {
    ...
    it('should show online status indicator for online friend', () => {
      const { container } = render(<FriendItem friend={mockOnlineFriend} index={0} />);
  ...
  describe('click handling', () => {
    it('should call onClick with friend data when clicked', () => {
      ...
      render(<FriendItem friend={mockOnlineFriend} index={0} onClick={handleClick} />);

      const item = screen.getByText('Alice Johnson').closest('div[class*="flex items-center"]');
  ...
  describe('different friend data', () => {
    ...
    it('should handle long lastSeen text', () => {
      ...
      expect(
        screen.getByText('Last seen a very long time ago on December 25th, 2023')
      ).toBeInTheDocument();
    });
  ...
  describe('combined scenarios', () => {
    ...
    it('should handle offline friend with click handler', () => {
      const handleClick = vi.fn();
      render(<FriendItem friend={mockOfflineFriend} index={0} onClick={handleClick} />);

      expect(screen.getByText('Bob Smith')).toBeInTheDocument();
      expect(screen.getByText('5 minutes ago')).toBeInTheDocument();

      const item = screen.getByText('Bob Smith').closest('div[class*="flex items-center"]');
      fireEvent.click(item);
    ...
    it('should render multiple friend items', () => {
      ...
      render(
        <>
          <FriendItem friend={mockOnlineFriend} index={0} onClick={handleClick} />
          <FriendItem friend={mockOfflineFriend} index={1} onClick={handleClick} />
        </>
      );
