...
const CommentInput = ({
  ...
  return (
    <div className="flex-shrink-0 p-4 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 transition-colors">
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="flex-1 min-w-0 flex gap-2">
          <input
            ...
            onKeyDown={onKeyDown}
            className="flex-1 min-w-0 px-3 sm:px-4 py-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 transition-colors"
          />
          <motion.button
            onClick={onAddComment}
            disabled={!commentText.trim()}
            className="flex-shrink-0 bg-orange-500 hover:bg-orange-600 text-white px-3 sm:px-4 py-2 rounded-full font-medium text-sm disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-orange-500 transition-colors whitespace-nowrap"
            whileHover={{
