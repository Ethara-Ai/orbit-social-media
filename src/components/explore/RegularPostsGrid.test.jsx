...
describe('RegularPostsGrid', () => {
  ...
  describe('ExplorePostCard props', () => {
    ...
    it('should pass correct index to each card', () => {
      render(<RegularPostsGrid {...defaultProps} />);
      expect(screen.getByTestId('explore-post-card-1')).toHaveAttribute('data-index', '0');
      expect(screen.getByTestId('explore-post-card-2')).toHaveAttribute('data-index', '1');
      expect(screen.getByTestId('explore-post-card-3')).toHaveAttribute('data-index', '2');
    });
  ...
  describe('empty posts array', () => {
    it('should render section with no cards when posts is empty', () => {
      ...
      expect(screen.getByText('Browse All')).toBeInTheDocument();
      expect(screen.queryByTestId(/explore-post-card-/)).not.toBeInTheDocument();
    });

    it('should still render grid container when posts is empty', () => {
      const { container } = render(<RegularPostsGrid posts={[]} onPostClick={vi.fn()} />);
      const grid = container.querySelector('.grid');
      expect(grid).toBeInTheDocument();
    });
  ...
  describe('single post', () => {
    it('should render correctly with single post', () => {
      ...
      expect(screen.getByTestId('explore-post-card-1')).toBeInTheDocument();
      expect(screen.queryByTestId('explore-post-card-2')).not.toBeInTheDocument();
    });
  ...
  describe('multiple posts', () => {
    ...
    it('should pass correct indices for many posts', () => {
      ...
      for (let i = 0; i < 5; i++) {
        expect(screen.getByTestId(`explore-post-card-${i + 1}`)).toHaveAttribute(
          'data-index',
          `${i}`
        );
      }
  ...
  describe('key prop', () => {
    it('should use post.id as key for each card', () => {
      ...
      render(<RegularPostsGrid posts={postsWithDuplicateTitles} onPostClick={vi.fn()} />);
      expect(screen.getByTestId('explore-post-card-unique-1')).toBeInTheDocument();
      expect(screen.getByTestId('explore-post-card-unique-2')).toBeInTheDocument();
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle posts with special characters in title', () => {
      ...
      expect(screen.getByText('Post with <special> & characters!')).toBeInTheDocument();
    });
      ...
      const longTitlePosts = [
        {
          ...mockPosts[0],
          id: 'long',
          title: 'This is a very long title that should still render correctly in the grid',
        },
      ];
      render(<RegularPostsGrid posts={longTitlePosts} onPostClick={vi.fn()} />);
      expect(
        screen.getByText('This is a very long title that should still render correctly in the grid')
      ).toBeInTheDocument();
    });
