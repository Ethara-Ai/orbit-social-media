...
// Mock icons
vi.mock('../icons', () => ({
  Fire: ({ className }) => <svg data-testid="fire-icon" className={className} />,
}));
...
describe('FeaturedSection', () => {
  ...
  describe('empty posts array', () => {
    it('should render section with no cards when posts is empty', () => {
      ...
      expect(screen.getByText('Trending Now')).toBeInTheDocument();
      expect(screen.queryByTestId(/featured-post-card-/)).not.toBeInTheDocument();
    });

    it('should still render grid container when posts is empty', () => {
      const { container } = render(<FeaturedSection posts={[]} onPostClick={vi.fn()} />);
      const grid = container.querySelector('.grid');
      expect(grid).toBeInTheDocument();
    });
  });
  ...
  describe('single post', () => {
    it('should render correctly with single post', () => {
      ...
      expect(screen.getByTestId('featured-post-card-1')).toBeInTheDocument();
      expect(screen.queryByTestId('featured-post-card-2')).not.toBeInTheDocument();
    });
  });
  ...
  describe('multiple posts', () => {
    ...
    it('should pass correct indices for many posts', () => {
      ...
      render(<FeaturedSection posts={manyPosts} onPostClick={vi.fn()} />);
      expect(screen.getByTestId('featured-post-card-1')).toHaveAttribute('data-index', '0');
      expect(screen.getByTestId('featured-post-card-2')).toHaveAttribute('data-index', '1');
      expect(screen.getByTestId('featured-post-card-3')).toHaveAttribute('data-index', '2');
    });
  });
  ...
  describe('key prop', () => {
    it('should use post.id as key for each card', () => {
      ...
      render(<FeaturedSection posts={postsWithDuplicateTitles} onPostClick={vi.fn()} />);
      expect(screen.getByTestId('featured-post-card-unique-1')).toBeInTheDocument();
      expect(screen.getByTestId('featured-post-card-unique-2')).toBeInTheDocument();
    });
  });
